import type { MarketDataProvider, ProviderChartPoint, ProviderMarket } from '../provider';
...
export const publicMarketProvider: MarketDataProvider = {
  ...
  async getChart(externalId, days) {
    const chart = await request<ChartResponse>(
      `/coins/${encodeURIComponent(externalId)}/market_chart?vs_currency=usd&days=${days}`,
    );
    return chart.prices.map(([timestamp, price]): ProviderChartPoint => ({ timestamp, price }));
  },
};
