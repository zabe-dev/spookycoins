...
export default function Home() {
  const [marketCoins, setMarketCoins] = useState<Coin[]>(fallbackCoins);
  ...
  useEffect(() => {
    ...
    fetch('/api/market/coins?limit=100', { signal: controller.signal })
      .then((response) => {
        ...
        return response.json() as Promise<{ data: Coin[] }>;
      })
      .then(({ data }) => {
          ...
          <div className="hotspots-viewport">
            ...
            >
              <Discovery
                ...
                coins={marketCoins.slice(-5).reverse()}
              />
              <Discovery
                icon="trend"
                title="Trending now"
                sub="Fastest-rising signals"
                coins={[...marketCoins].sort((a, b) => b.trend - a.trend).slice(0, 5)}
              />
              <Discovery
                icon="watch"
                title="Most watched"
                sub="Saved to portfolios"
                coins={marketCoins.slice(0, 5)}
              />
            </div>
