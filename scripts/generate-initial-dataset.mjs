...
const outputPath = resolve('features/coins/data/initial-dataset.generated.ts');
...
const realCoins = [...firstPage, ...secondPage]
...
if (realCoins.length !== 99) {
  throw new Error(`Expected 99 supported token coins, received ${realCoins.length}.`);
}

const promotionEndsAt = new Date(
  new Date(populatedAt).getTime() + 7 * 24 * 60 * 60 * 1000,
).toISOString();
const dummyPromotedCoin = {
  id: 1000,
  ...
  logoUrl: null,
  description: 'Demonstration listing used to preview promoted-coin advertising.',
  category: 'Other',
  launchDate: null,
  submittedAt: populatedAt,
  populatedAt,
...
const coins = [dummyPromotedCoin, ...realCoins];
...
export const initialCoins = ${JSON.stringify(coins, null, 2)} satisfies Coin[];
`;

await writeFile(outputPath, source, 'utf8');
console.log(`Generated ${coins.length} canonical token coins at ${outputPath}.`);
